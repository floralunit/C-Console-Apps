...
int main()
{
      ...
      const char* str = "";
      int hundreds = 0, thousands = 0, millions = 0, milliards = 0, lastNum, lastPenny;
      char *hChar = "", *tChar = "", *milChar = "", *mildChar = "", *nameNum, *pennyChar, *namePenny, *nameRub;
      if (num > 0) {
            hundreds = num % 1000;
      ...
      if (hundreds == 0)
        nameRub = "ноль рублей, ";
      else if (lastNum == 1 && hundreds % 100 != 11)
        nameRub = "рубль, ";
      else if (lastNum >= 2 && lastNum <= 4 && hundreds % 100 != 12)
        nameRub = "рубля, ";
      else 
        nameRub = "рублей, ";
      ...
      lastPenny = penny % 10;
      if (penny == 0)
        namePenny = "ноль копеек";
      else if (lastPenny == 1 && penny != 11)
        namePenny = "копейка ";
      else if (lastPenny >= 2 && lastPenny <= 4 && penny != 12)
        namePenny = "копейки ";
      else 
        namePenny = "копеек ";
      pennyChar = concat(nameRub, NumToString(penny, 1), namePenny, "");
      printf("%s", pennyChar);
    return 0;
}
